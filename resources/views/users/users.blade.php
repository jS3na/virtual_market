<x-main_layout pageTitle="Users">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between px-4 py-6">
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 w-full">
            <h1 class="text-2xl md:text-4xl font-semibold text-gray-800">Dashboard / Users</h1>
        </div>

        <form action="{{ route('users.search') }}" method="get" class="flex items-center gap-4 w-full max-w-4xl">
            <div class="flex w-full gap-2">
                <select name="role_id" class="p-2 border border-gray-300 rounded-l-lg bg-gray-100 dark:bg-gray-600 dark:text-white w-full">
                ...
                <a class="ml-2 px-4 py-2 bg-blue-900 text-white rounded-md hover:bg-blue-500" href="{{ route('users.add') }}">New</a>
                @endcan
            </div>
        </form>
    </div>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    @if(Gate::allows('has_permissions', 'users.update') || Gate::allows('has_permissions', 'users.delete'))
            ...
            <tbody>
                ...
                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                    ...
                    <td class="px-6 py-4 flex flex-row gap-4">
                        @can('has_permissions', 'users.update')
                        <a href="{{ route('users.edit', ['user_id' => $user->id]) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                        @endcan
                        @can('has_permissions', 'users.delete')
