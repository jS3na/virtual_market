<x-main_layout pageTitle="Roles">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between px-4 py-6">
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 w-full">
            <h1 class="text-2xl md:text-4xl font-semibold text-gray-800">Dashboard / Roles</h1>
        </div>

        <form action="{{ route('roles.search') }}" method="get" class="flex items-center gap-4 w-full max-w-4xl">
            <div class="flex w-full gap-2">
                ...
                @can('has_permissions', 'roles.create')
                <a class="ml-2 px-4 py-2 bg-blue-900 text-white rounded-md hover:bg-blue-500" href="{{ route('roles.add') }}">New</a>
                @endcan
            </div>
        </form>
    </div>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    @if(Gate::allows('has_permissions', 'roles.update') || Gate::allows('has_permissions', 'roles.delete'))
            ...
            <tbody>
                ...
                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                    ...
                    <td class="px-6 py-4 flex flex-row gap-4">
                        @can('has_permissions', 'roles.update')
                        <a href="{{ route('roles.edit', ['role_id' => $role->id]) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                        @endcan
                        @can('has_permissions', 'roles.delete')
